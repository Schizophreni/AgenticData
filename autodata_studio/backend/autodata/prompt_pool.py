...
ICONQA_PROMPTS: dict[str, PromptSpec] = {
    ...
    "fraction": PromptSpec(
        ...
        "in the options. Every supplied attachment must participate in the derived-ratio decision; "
        "never silently omit an image. Limit the solution to two arithmetic/comparison steps. "
        "Diversify the decision structure across: (1) a true cross-image ratio comparison "
        "statement; "
        "(2) a complete ordering of all named image ratios; (3) the median ratio or unique ratio "
        "outlier among at least three images; or (4) the image pair with equal ratios or the "
        "smallest absolute ratio difference. For pair or median tasks, the stem must explicitly "
        "state that every supplied image ratio is considered, and every substantive option must "
        "name the relevant image or pair. Select solely by the derived shaded ratios: never add "
        "matching partition count, equal denominator, or raw shaded-piece count as another "
        "condition. Do not repeatedly default to 'which statement correctly compares'. Every fraction "
        "question must use both the shaded-part numerator and total-part denominator for each "
        "considered image. Never rank or select images by total partition count alone, including "
        "most/fewest or greatest/least partition questions. The stem MUST use one of the four "
        "derived-ratio structures above. Every substantive option must encode a cross-image ratio "
        "relation, a complete ordering, a median/outlier choice that depends on all named ratios, "
        "or a pair selected by equality/absolute ratio difference. Never ask which image or pair "
        "merely represents a stated "
